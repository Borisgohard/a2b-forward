...
validate_network_value cidr 192.0.2.4/24 4
validate_network_value cidr 2001:db8::2/64 6
reject wrong-cidr-family validate_network_value cidr 2001:db8::/64 4
reject invalid-cidr-prefix validate_network_value cidr 192.0.2.0/33 4
reject config-injection validate_network_value cidr '192.0.2.0/24; deny all;' 4
validate_network_value interface a2b0
for name in ../wg0 'wg 0' 1234567890123456 'wg0;id'; do
    reject "invalid-interface:$name" validate_network_value interface "$name"
done
validate_network_value endpoint vpn.example.org
validate_network_value endpoint 2001:db8::1
reject endpoint-port validate_network_value endpoint vpn.example.org:51820
reject endpoint-injection validate_network_value endpoint $'vpn.example.org\nPostUp = id'
