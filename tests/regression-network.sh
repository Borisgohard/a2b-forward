...
set -Eeuo pipefail
[[ "${CI:-}" == true || "${A2B_TEST_DISPOSABLE:-}" == yes ]] || {
    echo 'Use disposable CI/VM only; set A2B_TEST_DISPOSABLE=yes on a test VM.' >&2
    exit 1
}
cd "$(dirname "$0")/.."
if [[ "${A2B_TEST_ISOLATED:-}" != yes ]]; then
    ...
    exec unshare -n -- env A2B_TEST_ISOLATED=yes bash tests/regression-network.sh
fi
...
if iptables-save | grep -- '--to-destination 198.51.100.2:18080'; then exit 1; fi
...
if grep -q FOREIGN "$RULES_V4_FILE"; then exit 1; fi
restore_saved_rules
restore_saved_rules
for cmd in iptables ip6tables; do
...
LOCAL_PORT=26445 LISTEN_ADDR=192.0.2.1
python3 "$root/tests/echo_peer.py" serve 192.0.2.1 26445 foreign & pids+=("$!")
sleep 0.2
if (preflight_mapping 4 tcp nat); then echo 'NAT would hijack a foreign listener'; exit 1; fi
